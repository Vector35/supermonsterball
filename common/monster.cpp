...
void MonsterSpecies::Init()
{
	...
	MonsterSpecies* hoppy = new MonsterSpecies(" 🐸 ", "Hoppy", "A small frog that jumps right where it wants to go.",
		...
		vector<Move*>{charge}, grassBiome, 100, 15, 128);
	...
	MonsterSpecies* clucker = new MonsterSpecies(" 🐥 ", "Clucker", "The small chicken's most potent attacks don't do "
		...
		vector<Move*>{cyclone, charge}, grassBiome, 60, 40, 180);
	MonsterSpecies* chickling = new MonsterSpecies(" 🐣 ", "Chickling", "A tiny chicken barely hatched that is nearly "
		...
		vector<MonsterSpecies*>{clucker}, 12, vector<Move*>{peck, wingAttack}, vector<Move*>{charge},
		grassBiome, 500, 350, 255);
	...
	MonsterSpecies* windove = new MonsterSpecies("🌪🕊 ", "Windove", "It is able to control the power of the wind to "
		...
		vector<Move*>{diveBomb, wingAttack}, vector<Move*>{hurricane, cyclone}, cityBiome, 25, 15, 60);
	...
	MonsterSpecies* dovelett = new MonsterSpecies(" 🐦 ", "Dovelett", "A diminutive bird that looks fantastically "
		...
		vector<MonsterSpecies*>{birdove}, 12, vector<Move*>{peck, wingAttack}, vector<Move*>{charge},
		cityBiome, 1000, 750, 255);
	...
	MonsterSpecies* rattichewy = new MonsterSpecies("🧀🐀 ", "Rattichewy", "No matter where you hide or move the "
		...
		vector<MonsterSpecies*>{}, 0, vector<Move*>{bite, cut}, vector<Move*>{chomp}, cityBiome, 40, 35, 80);
	MonsterSpecies* rattitat = new MonsterSpecies(" 🐀 ", "Rattitat", "This annoying rodent's tell-tale noise reminds"
		...
		vector<MonsterSpecies*>{rattichewy}, 25, vector<Move*>{bite, tackle}, vector<Move*>{charge},
		cityBiome, 1000, 900, 255);
	...
	MonsterSpecies* woofer = new MonsterSpecies(" 🐕 ", "Woofer", "The larger dog sends shockwaves through you with "
		...
		vector<MonsterSpecies*>{}, 0, vector<Move*>{wub}, vector<Move*>{sonicBoom}, cityBiome, 20, 10, 80);
	MonsterSpecies* subhound = new MonsterSpecies(" 🐶 ", "Subhound", "This bloodhound sniffs back and forth "
		...
		vector<MonsterSpecies*>{woofer}, 50, vector<Move*>{wub, bite}, vector<Move*>{charge}, cityBiome, 150, 75, 128);
	...
	MonsterSpecies* spidra = new MonsterSpecies(" 🕷 ", "Spidra", "You might not see the small spider, but you'll "
		...
		vector<MonsterSpecies*>{webadeth}, 50, vector<Move*>{bugBite}, vector<Move*>{infect}, grassBiome, 200, 150, 160);
	...
	MonsterSpecies* drillboar = new MonsterSpecies(" 🐗 ", "Drillboar", "This goring, boring, boar is known for "
		...
		vector<MonsterSpecies*>{}, 0, vector<Move*>{dig, cut}, vector<Move*>{earthquake, charge}, grassBiome, 5, 3, 45);
	...
	MonsterSpecies* stingping = new MonsterSpecies(" 🦂 ", "Stingping", "The tricky part about the stinger on this "
		...
		vector<MonsterSpecies*>{}, 0, vector<Move*>{sting, bugBite}, vector<Move*>{infect, poisonFang},
		desertBiome, 15, 5, 45);
	MonsterSpecies* antler = new MonsterSpecies(" 🐜 ", "Antler", "This small bug is well known for attacking deer "
		...
		vector<MonsterSpecies*>{stingping}, 50, vector<Move*>{sting, bugBite}, vector<Move*>{chomp},
		desertBiome, 200, 75, 180);
	...
	MonsterSpecies* wabbitwap = new MonsterSpecies("🕳🐇 ", "Wabbitwap", "I knew I shoulda taken that left turn at "
		...
		vector<MonsterSpecies*>{}, 0, vector<Move*>{trap, dig}, vector<Move*>{earthquake, quicksand},
		grassBiome, 10, 4, 60);
	...
	MonsterSpecies* glostar = new MonsterSpecies("🌟🌟 ", "Glostar", "A brilliant star that overshadows "
		...
		vector<MonsterSpecies*>{}, 0, vector<Move*>{starlight}, vector<Move*>{laser, nova, heatWave},
		mountainBiome, 5, 1, 10);
	MonsterSpecies* costar = new MonsterSpecies("⭐⭐ ", "Costar", "Always stuck in the shadow of something brighter, "
		...
		vector<MonsterSpecies*>{glostar}, 100, vector<Move*>{starlight}, vector<Move*>{heatWave, laser},
		mountainBiome, 30, 5, 45);
	MonsterSpecies* starem = new MonsterSpecies(" ⭐ ", "Starem", "A pretty, twinkling star.", Light, Light, 90, 90, 90,
		vector<MonsterSpecies*>{costar}, 25, vector<Move*>{starlight}, vector<Move*>{heatWave},
		mountainBiome, 150, 25, 100);
	...
	MonsterSpecies* electrikloud = new MonsterSpecies(" 🌩 ", "Electrikloud", "A dangerous cloud capable of both "
		...
		vector<Move*>{flood, heavyRain}, waterBiome, 40, 25, 50);
	...
	MonsterSpecies* snowmo = new MonsterSpecies("❄⛄ ", "Snowmo", "If you ask him for mercy, he'll just tell you "
		...
		vector<Move*>{freeze, frostbite}, vector<Move*>{blizzard, snowmageddon}, mountainBiome, 5, 1, 10);
	MonsterSpecies* snowblo = new MonsterSpecies("🌪⛄ ", "Snowblo", "The air that swirls around this snowman is "
		...
		vector<Move*>{freeze, frostbite}, vector<Move*>{snowstorm, blizzard}, mountainBiome, 30, 3, 45);
	MonsterSpecies* snowbro = new MonsterSpecies(" ⛄ ", "Snowbro", "A very chill snowman who seems to like just "
		...
		vector<MonsterSpecies*>{snowblo}, 25, vector<Move*>{icePick}, vector<Move*>{coldSnap},
		mountainBiome, 200, 10, 120);
	...
	MonsterSpecies* shellkra = new MonsterSpecies(" 🐚 ", "Shellkra", "If you think you like popping shells, you "
		...
		vector<MonsterSpecies*>{krabber}, 50, vector<Move*>{cut, slice}, vector<Move*>{heavyRain},
		waterBiome, 150, 10, 160);
	...
	MonsterSpecies* oktokore = new MonsterSpecies(" 🐙 ", "Oktokore", "It has eight brains and has been known to "
		...
		vector<Move*>{steamJet, wrap}, vector<Move*>{poisonFang, boneBreak}, waterBiome, 80, 8, 80);
	...
	MonsterSpecies* snukinasnail = new MonsterSpecies(" 🐌 ", "Snukinasnail", "It slowly moves in the shadows but "
		...
		vector<Move*>{sneakAttack}, vector<Move*>{backstab, chomp}, grassBiome, 75, 45, 45);
	...
	MonsterSpecies* lazybug = new MonsterSpecies(" 🐞 ", "Lazybug", "This bug is too lazy to move, so it uses its "
		...
		vector<Move*>{bite, bugBite}, vector<Move*>{memoryCorruption, infestation}, cityBiome, 125, 80, 45);
	...
	MonsterSpecies* gatorath = new MonsterSpecies("💥🐊 ", "Gatorath", "An angry gator is never a pleasant foe. And "
		...
		vector<MonsterSpecies*>{}, 0, vector<Move*>{bite, waterBlast}, vector<Move*>{chomp, charge},
		waterBiome, 10, 2, 10);
	MonsterSpecies* gatorate = new MonsterSpecies(" 🐊 ", "Gatorate", "If you stop to ask what the gatorate, the "
		...
		vector<MonsterSpecies*>{gatorath}, 50, vector<Move*>{bite}, vector<Move*>{chomp}, waterBiome, 60, 20, 45);
	...
	MonsterSpecies* chiptune = new MonsterSpecies(" 🐿 ", "Chiptune", "The strange noises that come from this "
		...
		vector<MonsterSpecies*>{}, 0, vector<Move*>{sawtooth}, vector<Move*>{sonicBoom}, grassBiome, 50, 30, 80);
	...
	MonsterSpecies* cowabucka = new MonsterSpecies(" 🐮 ", "Cowabucka", "If you try to go for a ride, you might find "
		...
		vector<MonsterSpecies*>{cowabusta}, 50, vector<Move*>{tackle}, vector<Move*>{charge}, grassBiome, 120, 50, 160);
	...
	MonsterSpecies* monkadoo = new MonsterSpecies(" 🐒 ", "Monkadoo", "This odorous monkey appears to be fighting "
		...
		vector<MonsterSpecies*>{}, 0, vector<Move*>{smash}, vector<Move*>{vileSludge}, desertBiome, 15, 3, 45);
	...
	MonsterSpecies* leptear = new MonsterSpecies(" 🐆 ", "Leptear", "Try and run, but this feline menace will hunt "
		...
		vector<MonsterSpecies*>{}, 0, vector<Move*>{cut, bite}, vector<Move*>{furyClaw, shadowClaw},
		cityBiome, 2, 1, 50);
	...
	MonsterSpecies* kitease = new MonsterSpecies(" 🐱 ", "Kitease", "An inviting cat that looks fun to play with but "
		...
		vector<MonsterSpecies*>{kitear}, 25, vector<Move*>{cut, slice}, vector<Move*>{shadowClaw},
		cityBiome, 300, 150, 255);
	...
	MonsterSpecies* shroomdoom = new MonsterSpecies(" 🍄 ", "Shroomdoom", "Anyone thinking that eating this will make"
		...
		vector<MonsterSpecies*>{}, 0, vector<Move*>{venomShock}, vector<Move*>{infestation, infect},
		grassBiome, 75, 45, 60);
	...
	MonsterSpecies* spiriboo = new MonsterSpecies(" 👻 ", "Spiriboo", "Attracted to abandoned buildings. It enjoys "
		...
		vector<Move*>{ghostBlade}, vector<Move*>{shadowClaw, backstab}, cityBiome, 80, 1, 60);
	...
	MonsterSpecies* ogrim = new MonsterSpecies(" 👹 ", "Ogrim", "For such a fierce ogre, this monster looks a little "
		...
		vector<MonsterSpecies*>{ogreat}, 50, vector<Move*>{punch, tackle}, vector<Move*>{charge},
		mountainBiome, 150, 35, 120);
	...
	MonsterSpecies* pilapoo = new MonsterSpecies(" 💩 ", "Pilapoo", "A sludge of incredibly vile nature gained a "
		...
		vector<Move*>{venomShock}, vector<Move*>{vileSludge, infect}, cityBiome, 100, 35, 60);
	...
	MonsterSpecies* flaret = new MonsterSpecies("🔥👽 ", "Flaret", "This alien form is most comfortable when aflame.",
		...
		vector<Move*>{heatWave, flamethrower, greatBallsOfFire}, desertBiome, 5, 1, 45);
	MonsterSpecies* electret = new MonsterSpecies("⚡👽 ", "Electret", "This alien form is at home with electrical "
		...
		vector<MonsterSpecies*>{}, 0, vector<Move*>{zap}, vector<Move*>{lightningBolt, jumpStart}, cityBiome, 5, 1, 45);
	MonsterSpecies* pouret = new MonsterSpecies("🌊👽 ", "Pouret", "This alien form is adapted to aquatic regions.",
		...
		vector<Move*>{waterfall, heavyRain, tsunami}, waterBiome, 5, 1, 45);
	MonsterSpecies* freezet = new MonsterSpecies("❄👽 ", "Freezet", "This alien form is adapted to cold climates.",
		...
		vector<Move*>{coldSnap, blizzard, snowmageddon}, mountainBiome, 5, 1, 45);
	MonsterSpecies* pixet = new MonsterSpecies(" 👾 ", "Pixet", "This alien form seems to blend into video games quite "
		...
		vector<MonsterSpecies*>{}, 0, vector<Move*>{sawtooth}, vector<Move*>{sonicBoom, laser, nova},
		grassBiome, 5, 1, 45);
	MonsterSpecies* eetee = new MonsterSpecies(" 👽 ", "Eetee", "An alien with an exotic genetic structure capable of "
		...
		vector<Move*>{tackle, bite}, vector<Move*>{charge}, cityBiome, 125, 125, 180);
	...
	MonsterSpecies* wyrmton = new MonsterSpecies("🌊🐉 ", "Wyrmton", "A giant dragon that weighs over 2000lbs.",
		...
		vector<Move*>{tsunami, hurricane, cyclone}, waterBiome, 10, 1, 3);
	MonsterSpecies* phishy = new MonsterSpecies(" 🐟 ", "Phishy", "A useless fish that can't do much of anything.",
		...
		vector<Move*>{struggle}, waterBiome, 250, 5, 255);
	...
	MonsterSpecies* botto = new MonsterSpecies(" 🤖 ", "Botto", "Friendly helper or obnoxious pest? You decide.",
		...
		vector<MonsterSpecies*>{bottabuzz}, 50, vector<Move*>{zap, cut}, vector<Move*>{charge}, cityBiome, 75, 5, 120);
	...
	MonsterSpecies* sundaze = new MonsterSpecies(" ☀ ", "Sundaze", "The glare from this glowing orb is hard to see "
		...
		vector<MonsterSpecies*>{ohdaze}, 100, vector<Move*>{sunBeam}, vector<Move*>{nova}, desertBiome, 120, 40, 120);
	...
	MonsterSpecies* liarkake = new MonsterSpecies(" 🎂 ", "Lyarkake", "Fills the mind of anything around it with "
		...
		vector<Move*>{typeConfusion}, vector<Move*>{mindMelt, memoryCorruption}, cityBiome, 40, 10, 45);
	...
	MonsterSpecies* tacocat = new MonsterSpecies(" 🌮 ", "Tacocat", "Was it a cat I saw? Looks like a delicious taco to draw "
		...
		vector<Move*>{hotSauce, heatWave}, cityBiome, 40, 10, 45);
	...
	MonsterSpecies* tuxacool = new MonsterSpecies(" 🐧 ", "Tuxacool", "Not always easy to work with, but you can't "
		...
		vector<MonsterSpecies*>{}, 0, vector<Move*>{icePick, peck}, vector<Move*>{coldSnap, blizzard},
		waterBiome, 90, 25, 50);
	...
	MonsterSpecies* crystrike = new MonsterSpecies(" 💎 ", "Crystrike", "A multifaceted and deadly shimmering gem.",
		...
		vector<MonsterSpecies*>{}, 0, vector<Move*>{typeConfusion}, vector<Move*>{laser, magicMissile},
		mountainBiome, 90, 25, 45);
	...
	MonsterSpecies* galaxseven = new MonsterSpecies(" 📱 ", "Galaxseven", "A product release gone horribly wrong "
		...
		vector<Move*>{shortCircuit, glassCannon}, vector<Move*>{batteryFire, handGrenade}, cityBiome, 150, 5, 60);
	...
	MonsterSpecies* burninator = new MonsterSpecies("🔥🐲 ", "Burninator", "It likes to burninate the peasants "
		...
		vector<Move*>{fireball, wingAttack}, vector<Move*>{burninate, flamethrower, greatBallsOfFire}, grassBiome, 1, 1, 3);
	MonsterSpecies* slithaburn = new MonsterSpecies("🔥🐍 ", "Slithaburn", "This forked tongue spits fire with its "
		...
		vector<Move*>{flare, bite}, vector<Move*>{heatWave, poisonFang}, grassBiome, 8, 3, 10);
	MonsterSpecies* slither = new MonsterSpecies(" 🐍 ", "Slither", "Easily mistaken for a garden hose, but don't "
		...
		vector<MonsterSpecies*>{slithaburn}, 25, vector<Move*>{bite}, vector<Move*>{poisonFang, chomp}, grassBiome, 30, 5, 45);
	...
	MonsterSpecies* bearly = new MonsterSpecies(" 🐻 ", "Bearly", "An obtuse bear, ignorant of puns, that when asked "
		...
		vector<MonsterSpecies*>{ehkaybear}, 100, vector<Move*>{bite}, vector<Move*>{chomp, charge}, grassBiome, 75, 15, 45);
...
BallThrowResult Monster::GetThrowResult(ItemType ball, bool seedGiven)
{
	uint32_t catchRate = m_species->GetCatchRate();
	uint32_t fleeRate = catchRate / 2;
	if (m_level >= 30)
		catchRate /= 3;
	else
		catchRate = (((3 * 30) - (2 * m_level)) * catchRate) / (3 * 30);
	if (catchRate < 1)
		catchRate = 1;
	if (fleeRate < 1)
		fleeRate = 1;
	...
	uint32_t catchThreshold = (uint32_t)(65536.0f / pow(255.0f / (float)catchRate, 1.0f / 5.0f));
	uint32_t fleeThreshold = (uint32_t)(65536.0f / pow(255.0f / (float)fleeRate, 1.0f / 5.0f));

	if ((rand() & 0xffff) >= catchThreshold)
	{
		bool flee = true;
		for (size_t i = 0; i < 4; i++)
		{
			if ((rand() & 0xffff) >= fleeThreshold)
				flee = false;
		}
		if (flee)
